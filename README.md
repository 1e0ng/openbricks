...
- **Assembly Workbench (`openbricks sim workbench`).** Build the robot in the browser from LEGO Technic bricks with their exact LDraw geometry — pins and axles snap into holes — or from your own STL parts; components nest, and mass, centre of mass and inertia are computed from the real shapes at every level. `openbricks bricks fetch` brings in the whole LDraw library on demand.
