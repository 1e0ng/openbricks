...
- **The sim (`openbricks sim`).** A native desktop app: build the robot from LEGO Technic bricks with their exact LDraw geometry — pins and axles snap into holes — or from your own STL parts; components nest, and mass, centre of mass and inertia are computed from the real shapes at every level. The first run downloads the signed build for your platform; `openbricks sim workbench` is the same editor in a browser, and `openbricks bricks fetch` brings in the whole LDraw library on demand.
