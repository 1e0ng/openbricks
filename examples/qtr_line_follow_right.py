...
"""Line following on the split 5+5 QTR rig — RIGHT-edge discipline.

Run ``examples/qtr_calibrate.py`` once first. The RIGHT cluster
follows the line's RIGHT edge; the LEFT cluster is the branch /
ending flag bank. Right cluster fully dark AND the left cluster
...
def get_wheel_speeds(reading, branch_dark):
    if branch_dark and reading.all_dark():
        return None
    steer = KP * reading.right_edge_position()
...
print("following the RIGHT edge. Intersection stops the run.")
while True:
    reading = right_qtr.read()
    branch_dark = left_qtr.read().dark_count() > 0
