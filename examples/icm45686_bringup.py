...
"""
ICM-45686 bring-up — verify a freshly wired breakout layer by layer.

Wire the breakout to 4 free GPIOs (defaults below — edit to your
wiring), power at 3.3 V, then run this. It prints the evidence:

  1. WHO_AM_I answers 0xE9 (construction raises loudly if not —
     that means wiring, usually SDO/SDI swapped).
  2. Accel sanity: at rest expect ~(0, 0, +1 g).
...
Run:
    openbricks run -n <hub> examples/icm45686_bringup.py
"""
