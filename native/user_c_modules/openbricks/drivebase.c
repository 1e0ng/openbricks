...
static void drivebase_control_tick(void *ctx) {
    ...
    mp_float_t sum_pos  = (self->left->core.observer.pos_hat + self->right->core.observer.pos_hat) / (mp_float_t)2.0;
    mp_float_t diff_pos;
    if (self->use_gyro && self->imu_heading_fn != MP_OBJ_NULL) {
        ...
        diff_pos = (self->left->core.observer.pos_hat - self->right->core.observer.pos_hat) / (mp_float_t)2.0;
    }
    ...
    self->left->core.target_dps  = fwd_cmd + diff_cmd;
    self->right->core.target_dps = fwd_cmd - diff_cmd;
}
...
static void drivebase_register(drivebase_obj_t *self) {
    ...
    mp_float_t lp = (mp_float_t)lc * (mp_float_t)360.0 / (mp_float_t)self->left->core.counts_per_rev;
    mp_float_t rp = (mp_float_t)rc * (mp_float_t)360.0 / (mp_float_t)self->right->core.counts_per_rev;
    ob_observer_reset(&self->left->core.observer,  lp);
    ob_observer_reset(&self->right->core.observer, rp);
    self->left->core.last_time_ms  = openbricks_motor_process_now_ms();
    self->right->core.last_time_ms = openbricks_motor_process_now_ms();
    ...
    self->left->core.target_dps  = 0.0;
    self->right->core.target_dps = 0.0;
    self->left->core.traj_active  = false;
    self->right->core.traj_active = false;
    self->left->core.traj_done  = true;
    self->right->core.traj_done = true;
...
static mp_obj_t db_straight(mp_obj_t self_in, mp_obj_t distance_in, mp_obj_t speed_in) {
    ...
    mp_float_t sum_pos  = (self->left->core.observer.pos_hat + self->right->core.observer.pos_hat) / (mp_float_t)2.0;
    mp_float_t diff_pos = (self->left->core.observer.pos_hat - self->right->core.observer.pos_hat) / (mp_float_t)2.0;
