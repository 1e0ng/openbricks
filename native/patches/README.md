...
| `esp32-openbricks-bus-uart.patch` | Non-blocking IDF UART shims (`ob_bus_uart_open/tx/rx/flush_rx/close` + `MICROPY_OPENBRICKS_BUS_UART`) for the native serial-bus motor path: the hard-tick bus pump needs raw byte I/O that NEVER waits (TX refuses on a full ring instead of blocking; RX returns only what the driver ring holds). Consumed by `st_bus.c` via extern symbols. Anchored away from the hard-tick patch's hunks so both apply in any order. | openbricks-specific plumbing — not an upstream candidate. |
| `esp32-openbricks-gpio-shim.patch` | GPIO input shims (`ob_gpio_input_pullup`/`ob_gpio_read` + `MICROPY_OPENBRICKS_GPIO_SHIM`) for hard-tick consumers — the hard-button sampler reads the program button's level from the esp_timer task; `gpio_get_level` is a register read, safe at any rate from any task. | openbricks-specific plumbing — not an upstream candidate. |
