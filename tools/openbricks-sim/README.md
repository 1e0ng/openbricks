...
* **Phase D** 🚧 distance sensors + scenarios. `SimDistanceSensor` raycasts forward from the chassis `chassis_dist` site; `ShimHCSR04` / `ShimVL53L0X` / `ShimVL53L1X` all wrap it — `from openbricks.drivers.hcsr04 import HCSR04` (or `vl53l0x` / `vl53l1x`) works unchanged in the sim. `robot.reset()` and `robot.set_pose(x_mm, y_mm, yaw_deg)` let you iterate on a mission script without restarting the sim process. **Just shipped:** mission-scoring predicates — `chassis_in_box`, `chassis_in_circle`, `distance_to`, `heading_aligned_with`. See `examples/scored_mission.py` (a speed-sweep that scores each attempt) and `examples/repeat_mission.py`. Next: worlds library expansion.
